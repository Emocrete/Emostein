...
export async function POST({ request }) {
	try {
		...
		const Res = await SupabaseFetch("/rest/v1/replay_sessions?on_conflict=id", { method: "POST", headers: { "content-type": "application/json", prefer: "resolution=merge-duplicates,return=representation" }, body: JSON.stringify(Row) });
...
export async function GET({ request }) {
	try {
		...
		const Query = new URL("http://x/rest/v1/replay_sessions"); Query.searchParams.set("select", "*");
		if (Id) { Query.searchParams.set("id", `eq.${Id}`); Query.searchParams.set("limit", "1"); }
		else if (VisitSessionId) { Query.searchParams.set("visit_session_id", `eq.${VisitSessionId}`); Query.searchParams.set("order", "started_at.asc"); Query.searchParams.set("limit", "200"); }
