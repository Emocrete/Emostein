...
async function InsertControl(pSupabaseUrl, pServiceKey, pVisitorId, pSessionId, pClient) {
	const Now = new Date().toISOString();
	const Uid = `ops_delete_session_${Date.now()}_${Math.random().toString(36).slice(2, 10)}`;
	const Payload = { controlCommand: "ops_delete_session", targetVisitorId: pVisitorId, targetSessionId: pSessionId, createdAtClient: Now, eventSource: "ops" };
	const Row = {
		event_stream: "normal",
		client_event_uid: Uid,
		event_type: "ops_delete_session",
		event_source: "ops",
...
export async function POST({ request }) {
	try {
		...
		if (!ValidId(VisitorId, "v") || !ValidId(SessionId, "s")) return Json({ ok: false, error: "Invalid visitorId/sessionId" }, 400);
		const { SupabaseUrl, ServiceKey } = Config();
		const DeleteQuery = new URL(`${SupabaseUrl}/rest/v1/ops_events`);
		DeleteQuery.searchParams.set("session_id", `eq.${SessionId}`);
		await SupabaseRequest(DeleteQuery, ServiceKey, { method: "DELETE", headers: { prefer: "return=minimal" } });
		await InsertControl(SupabaseUrl, ServiceKey, VisitorId, SessionId, Client);
		return Json({ ok: true, visitorId: VisitorId, sessionId: SessionId });
	} catch (Ex) {
