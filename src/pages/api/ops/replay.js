...
function ResizeDeviceFrame() {
	...
	const RotatedControlThickness = RotateForMobilePreview ? 48 : 0;
...
function EventHasPointer(pEvent) {
	return PointerPointOf(pEvent) !== null;
}
...
function UpdateCursorForTime(pMs) {
	...
	const Gap = Math.max(1, NextTime - PrevTime);
	if (Gap > 900 && pMs < NextTime - 120) return MoveCursorTo(Prev, false);
	const Ratio = Math.max(0, Math.min(1, (pMs - PrevTime) / Gap));
	const Mixed = {
...
function BuildTimelineLayer() {
	...
	function AddSegment(StartMs, EndMs, ClassName) {
		...
		Elem.style.setProperty("--segTop", Left + "%");
		Elem.style.setProperty("--segHeight", Width + "%");
	...
	function AddMarker(TimeMs, Text, Title) {
		...
		Elem.title = Title || "";
		const Position = Math.max(0, Math.min(100, TimeMs / fMaxMs * 100));
		Elem.style.left = Position + "%";
		Elem.style.setProperty("--markerTop", Position + "%");
		fTrackLayer.appendChild(Elem);
	}
	let LastPage = "";
	for (let Index = 0; Index < cEvents.length; Index++) {
